...
const FinanceSidebar = ({ isCollapsed, setIsCollapsed }) => {
  ...
  const allMenuItems = [
    ...
    { name: 'Revenue Analytics', icon: <DollarSign size={20} />, path: '/finance/revenue' },
    ...
    { name: 'Pricing & Charges', icon: <Tag size={20} />, path: '/finance/pricing' },
    { name: 'Refund Management', icon: <Undo2 size={20} />, path: '/finance/refunds' },
    { name: 'Invoices & Billing', icon: <Receipt size={20} />, path: '/finance/invoices' },
    { name: 'Financial Reports', icon: <FileText size={20} />, path: '/finance/reports' },
  ];

  // Filter based on "view" permission
  const menuItems = allMenuItems.filter(item => {
    if (permissions.all || role === 'superAdmin') return true;
    const modPerms = permissions[item.name] || [];
    return modPerms.includes('view');
  });
